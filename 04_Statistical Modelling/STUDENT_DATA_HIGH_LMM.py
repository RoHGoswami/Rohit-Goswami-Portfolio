...
#  Load & Inspect
# ══════════════════════════════════════════════
data = pd.read_csv("Student_data_high.csv")
print(data.dtypes)
print(data.describe())
...
# ══════════════════════════════════════════════
# statsmodels MixedLM uses groups= for the random effect grouping variable
# We need to ensure Parental_Education_Level is string for grouping
...
# ══════════════════════════════════════════════
# LRT: -2 * (logLik_reduced - logLik_full) ~ chi²(df=1)
...
plt.tight_layout()
plt.savefig("high_qq_plot.png", dpi=150)
plt.show()

# Breusch-Pagan Test (using auxiliary OLS of squared residuals on fitted values)
bp_data = pd.DataFrame({"resid": residuals_full, "fitted": fitted_values})
bp_model = smf.ols("resid ~ fitted", data=bp_data).fit()
...
plt.tight_layout()
plt.savefig("high_scale_location.png", dpi=150)
plt.show()
...
var_random = float(log_model.cov_re.iloc[0, 0])  # random intercept variance
var_resid = log_model.scale  # residual variance
...
# ══════════════════════════════════════════════
# statsmodels MixedLM doesn't have drop1(), so we implement backward
# elimination by iteratively removing the least significant fixed effect.
...
while True:
    ...
    # Get p-values for fixed effects (exclude Intercept and Group Var)
    ...
    # Remove the variable from formula string
    # Handle interaction terms and C() wrapping
...
plt.tight_layout()
plt.savefig("high_fitted_vs_residuals.png", dpi=150)
plt.show()
